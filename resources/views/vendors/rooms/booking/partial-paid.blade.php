...
<div class="modal fade" id="paymentModal{{ $booking->id }}" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('vendor.room_bookings.update_partial_amount') }}" method="POST">
                @csrf
                <input type="hidden" name="booking_id" value="{{ $booking->id }}">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="total_amount">{{ __('Total Amount') }}
                            ({{ $currencyInfo->base_currency_text }})</label>
                        <input type="number"step="0.01" class="form-control" id="total_amount"
                            value="{{ $booking->grand_total }}" disabled>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="paying_amount">{{ __('Amount to Pay Now') . '*' }}
                            ({{ $currencyInfo->base_currency_text }})</label>
                        <input type="number" step="0.01" class="form-control" id="paying_amount"
                            name="paying_amount" required>
                    </div>
                </div>
