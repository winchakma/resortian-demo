...
@section('content')
    <div class="page-header">
        @if (request()->routeIs('admin.vendor_room_bookings.all_bookings'))
            <h4 class="page-title">{{ __('All Bookings') }}</h4>
        @elseif (request()->routeIs('admin.vendor_room_bookings.approved_bookings'))
            <h4 class="page-title">{{ __('Approved Bookings') }}</h4>
        @elseif (request()->routeIs('admin.vendor_room_bookings.pending_bookings'))
            <h4 class="page-title">{{ __('Pending Bookings') }}</h4>
        @elseif (request()->routeIs('admin.vendor_room_bookings.canceled_bookings'))
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row justify-content-between">
                        ...
                        <div class="col-lg-2">
                            <form
                                @if (request()->routeIs('admin.vendor_room_bookings.all_bookings')) action="{{ route('admin.vendor_room_bookings.all_bookings') }}"
                @elseif (request()->routeIs('admin.vendor_room_bookings.approved_bookings'))
                  action="{{ route('admin.vendor_room_bookings.approved_bookings') }}"
                @elseif (request()->routeIs('admin.vendor_room_bookings.canceled_bookings'))
                  action="{{ route('admin.vendor_room_bookings.canceled_bookings') }}"
                @elseif (request()->routeIs('admin.vendor_room_bookings.pending_bookings'))
                  action="{{ route('admin.vendor_room_bookings.pending_bookings') }}" @endif
                                method="GET">
                  ...
                  action="{{ route('admin.vendor_room_bookings.pending_bookings') }}" @endif
                        ...
                        <div class="col-lg-2">
                            <button class="btn btn-danger btn-sm float-right d-none bulk-delete ml-3 "
                                data-href="{{ route('admin.vendor_room_bookings.bulk_delete_booking') }}">
                                        ...
                                        <tbody>
                                            @foreach ($bookings as $booking)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if (!empty($booking->vendor))
                                                            <a
                                                                href="{{ route('admin.vendor_management.vendor_details', ['id' => $booking->vendor->id, 'language' => $defaultLanguageCode]) }}">
                                                        ...
                                                        @else
                                                            ...
                                                            @else
                                                                <form id="stayStatusForm{{ $booking->id }}"
                                                                    ...
                                                                    action="{{ route('admin.vendor_room_bookings.update_stay_status') }}"
                                                                    method="post">
                                                                    @csrf
                                                    ...
                                                    <td>
                                                        ...
                                                        @if ($booking->gateway_type == 'online')
                                                            ...
                                                            @elseif ($booking->payment_status == 3 || $booking->payment_status == 2)
                                                                ...
                                                                @endif
                                                                @includeIf('admin.rooms.vendor_booking.partial-paid')
                                                            @else
                                                                <h2 class="d-inline-block"><span
                                                        ...
                                                        @else
                                                            ...
                                                            @else
                                                                <form id="paymentStatusForm{{ $booking->id }}"
                                                                    class="d-inline-block"
                                                                    action="{{ route('admin.vendor_room_bookings.update_payment_status') }}"
                                                                    method="post">
                                                                    @csrf
                                                                    <input type="hidden" name="booking_id"
                                                                        value="{{ $booking->id }}">
                                                    ...
                                                    <td>
                                                        @if ($booking->booking_status != 2)
                                                            <form id="bookingStatusForm{{ $booking->id }}"
                                                                class="d-inline-block"
                                                                action="{{ route('admin.vendor_room_bookings.update_booking_status') }}"
                                                                method="post">
                                                                @csrf
                                                                <input type="hidden" name="booking_id"
                                                                    value="{{ $booking->id }}">
                                                            ...
                                                            @includeIf('admin.rooms.vendor_booking.make-refund')
                                                        @else
                                                    ...
                                                    <td>
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu"
                                                                ...
                                                                @if ($booking->booking_status != 2)
                                                                    <a href="{{ route('admin.vendor_room_bookings.booking_details', ['id' => $booking->id]) }}"
                                                                        class="dropdown-item">
                                                                        {{ __('Details') }}
                                                                    </a>
                                                                @endif
                                                                @if ($booking->booking_status != 2)
                                                                    <a href="{{ route('admin.vendor_room_bookings.booking_edit', ['id' => $booking->id]) }}"
                                                                        class="dropdown-item">
                                                                        {{ __('Edit') }}
                                                                    </a>
                                                                @endif
                                                                @if ($booking->booking_status != 2)
                                                                    <a href="{{ route('admin.vendor_room_bookings.booking_paid_services', ['id' => $booking->id]) }}"
                                                                        class="dropdown-item">
                                                                        {{ __('Paid Services') }}
                                                                    </a>
                                                                @endif
                                                                ...
                                                                <form class="deleteForm d-block"
                                                                    action="{{ route('admin.vendor_room_bookings.delete_booking', ['id' => $booking->id]) }}"
                                                                    method="post">
                                                                    @csrf
                                                ...
                                                @includeIf('admin.rooms.vendor_booking.show-attachment')
                                            @endforeach
                                        </tbody>
                                    </table>
                                </div>
    ...
    @includeIf('admin.rooms.vendor_booking.send-mail')
    @includeIf('admin.rooms.vendor_booking.all-room-categories')
@endsection
...
@section('script')
    <script>
        ...
        $(function() {
            if ($.fn.select2 && $('#selected-room').length) {
                $('#selected-room').select2({
                    width: '100%',
                    dropdownParent: $('#roomModal'),
                    placeholder: "{{ __('Select a Room Category') }}"
                });
            }
        });
