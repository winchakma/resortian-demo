...
@section('content')
          ...
          <div class="row">
            ...
            <div class="col-lg-3">
              ...
              <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
                data-href="{{ route('admin.rooms_management.paid_service.bulk_delete') }}"><i
                        ...
                        <tr>
                          ...
                          <td>
                            @if ($service->vendor_id != 0)
                              <a
                                href="{{ route('admin.vendor_management.vendor_details', ['id' => $service->vendor_id, 'language' => $defaultLang->code]) }}">{{ $vendor = optional($service->vendor)->username }}</a>
                            @else
                          ...
                          <td>
                            @if ($currencyInfo->base_currency_symbol_position == 'left')
                              {{ $currencyInfo->base_currency_symbol }} {{ number_format($service->price, 2) }}
                            @else
                              {{ number_format($service->price, 2) }} {{ $currencyInfo->base_currency_symbol }}
                            @endif
                          </td>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm mr-1 editBtn" href="#" data-toggle="modal"
                              ...
                              data-status="{{ $service->status }}" data-price="{{ $service->price }}"
                            ...
                            <form class="deleteForm d-inline-block"
                              action="{{ route('admin.rooms_management.paid_service.delete') }}" method="post">
                              @csrf
  ...
  {{-- create modal --}}
  @include('admin.rooms.paid-services.create')

  {{-- edit modal --}}
  @include('admin.rooms.paid-services.edit')
@endsection
