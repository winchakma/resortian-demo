...
@section('content')
  <div class="page-header">
    @if (request()->routeIs('admin.check_outs.delayed'))
      <h4 class="page-title">{{ __('Delayed') }}</h4>
    @elseif (request()->routeIs('admin.check_outs.upcoming'))
        ...
        <div class="card-header">
          <div class="row">
            ...
            <div class="col-xl-12">
              ...
              <form class="from-wrapper" id="Form"
                @if (request()->routeIs('admin.check_outs.delayed')) action="{{ route('admin.check_outs.delayed') }}"
                @elseif (request()->routeIs('admin.check_outs.upcoming')) action="{{ route('admin.check_outs.upcoming') }}" @endif
                method="GET">
                <div class="form-input-row">
                  <select name="vendor_id" class="form-control select2"
                    ...
                    @foreach ($vendors as $vendor)
                      <option @selected(request()->input('vendor_id') == $vendor->id) value="{{ $vendor->id }}">{{ $vendor->username }}
                      </option>
                    @endforeach
                  </select>
                  {{-- Booking No --}}
                  <input name="booking_no" type="text" class="form-control min-w200" placeholder="Search By Booking No"
                    value="{{ request('booking_no', '') }}">
                  <input name="keyword" id="keyword" type="text" class="form-control min-w250"
                    placeholder="Search By name / phone / email" value="{{ request()->input('keyword') ?? '' }}">

                  {{-- Date Option --}}
                  <select id="date_option" name="date_option" class="form-control" onchange="handleDateOptionChange()">
                    @if (request()->routeIs('admin.check_outs.upcoming'))
                            ...
                            @else
                              ...
                              @else
                                <form id="stayStatusForm{{ $booking->id }}" class="d-inline-block"
                                  action="{{ route('admin.room_bookings.update_stay_status') }}" method="post">
                                  @csrf
...
<script>
  function handleDateOptionChange() {
    ...
    const isUpcoming = {{ request()->routeIs('admin.check_outs.upcoming') ? 'true' : 'false' }};
