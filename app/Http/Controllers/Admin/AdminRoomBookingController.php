...
{
  private function adminRoomBookingQuery()
  {
    return RoomBooking::query()->where('vendor_id', 0);
  }

  private function adminRoomBookingOrFail($id)
  {
    return $this->adminRoomBookingQuery()->findOrFail($id);
  }

  private function adminRoomCategoryQuery()
  {
    return Room::query()->where('vendor_id', 0);
  }

  private function adminRoomCategoryOrFail($id)
  {
    return $this->adminRoomCategoryQuery()->findOrFail($id);
  }
  ...
  public function index(Request $request)
  {
    ...
    if (URL::current() == Route::is('admin.room_bookings.approved_bookings')) {
      $booking_status = "approved";
    } else if (URL::current() == Route::is('admin.room_bookings.pending_bookings')) {

      $booking_status = "pending";
    } else if (URL::current() == Route::is('admin.room_bookings.canceled_bookings')) {

      $booking_status = "canceled";
    }

    $now = Carbon::now(config('app.timezone'));
    $bs = DB::table('basic_settings')->select('checkin_time', 'checkout_time')->first();
    $checkinTime = $bs->checkin_time;
    $checkOutTime = $bs->checkout_time;

    $information['bookings'] = RoomBooking::where('vendor_id', 0)->when($booking_number, function ($query, $booking_number) {
      ...
      ->when(URL::current() == route('admin.room_bookings.active_bookings'), function ($query) use ($now, $checkinTime, $checkOutTime) {
        return $query->where('booking_status', '!=', 2)
    ...
    return view('admin.rooms.booking.index', $information);
  }
  public function todaysBooked(Request $request)
  {
    ...
    $bookedRooms = DB::table('room_numbers as rn')
      ->join('room_categories as r', 'rn.room_category_id', '=', 'r.id')
    ...
    return view('admin.rooms.booking.todays-booked', $information);
  }
  public function checkIn(Request $request)
  {
    ...
    if ($vendor_id === 'admin') {
      $bs = DB::table('basic_settings')->select('checkin_time')->first();
    } else {
      $bs = Vendor::where('id', $vendor_id)->select('checkin_time')->first();
    }
    ...
    $bookings = RoomBooking::query()
      ...
      ->when($request->routeIs('admin.check_ins.delayed'), function ($q) use ($request, $date_option, $checkinExpr, $checkinTime, $now) {
      ...
      ->when($request->routeIs('admin.check_ins.upcoming'), function ($q) use ($request, $date_option, $checkinExpr, $checkinTime, $now) {
    ...
    return view('admin.rooms.booking.check-in', $information);
  }
  public function checkOut(Request $request)
  {
    ...
    if ($vendor_id === 'admin') {
      $bs = DB::table('basic_settings')->select('checkout_time')->first();
    } else {
      $bs = Vendor::where('id', $vendor_id)->select('checkout_time')->first();
    }
    ...
    $bookings = RoomBooking::query()
      ...
      ->when($request->routeIs('admin.check_outs.delayed'), function ($q) use ($request, $checkoutExpr, $checkOutTime, $now, $date_option) {
      ...
      ->when($request->routeIs('admin.check_outs.upcoming'), function ($q) use ($request, $checkoutExpr, $checkOutTime, $now, $date_option) {
    ...
    return view('admin.rooms.booking.check-out', $information);
  }


  public function updatePaymentStatus(Request $request)
  {
    $bookingInfo = $this->adminRoomBookingOrFail($request->booking_id);
  ...
  {
    $booking = $this->adminRoomBookingOrFail($request->booking_id);
    ...
    // ===============================
    $room = Room::find($booking->room_category_id);
  ...
  {
    $roomBooking = $this->adminRoomBookingOrFail($request->booking_id);
  ...
  public function refunds()
  {
    $information['refunds'] = Refund::query()
      ->where('vendor_id', 0)
      ->whereIn('status', [0, 4, 5])
      ->orderBy('id', 'desc')
      ->get();

    $information['currencyInfo'] = MiscellaneousTrait::getCurrencyInfo();

    return view('admin.rooms.booking.refunds', $information);
  }
  ...
  public function updateRefundStatus(Request $request)
  {
    ...
    $booking = $this->adminRoomBookingQuery()->find($refund->booking_id);

    //If admin approved refund
    if ($newStatus === 4) {
      $booking = $this->adminRoomBookingOrFail($refund->booking_id);
  ...
  public function disputes()
  {
    ...
    return view('admin.rooms.booking.disputes', $information);
  }


  public function editBooking($id)
  {
    $details = $this->adminRoomBookingOrFail($id);
    ...
    return view('admin.rooms.booking.edit', $information);
  }
  ...
  public function updateBooking(AdminRoomBookingRequest $request)
  {
    ...
    // Calculate rent
    $roomCategory = $this->adminRoomCategoryOrFail($request->room_category_id);
  ...
  public function bookingDetails($id)
  {
    ...
    return view('admin.rooms.booking.details', $information);
  }
  public function paidServices($id)
  {
    ...
    return view('admin.rooms.booking.paid-services', $information);
  }
  ...
  {
    $roomBooking = $this->adminRoomBookingOrFail($id);
  ...
  {
    $quantity = $this->adminRoomCategoryOrFail($id)->quantity;
    ...
    if (is_null($bookingId)) {
      ...
      $booking = $this->adminRoomBookingOrFail($bookingId);
  ...
  public function bookedDates(Request $request)
  {
    ...
    return response()->json([
      'success' => route('admin.room_bookings.booking_form', [
  ...
  public function bookingForm(Request $request)
  {
    ...
    $room = $this->adminRoomCategoryOrFail($id);
    ...
    return view('admin.rooms.booking.booking-form', $information);
  }
  ...
  public function totalRooms(Request $request)
  {
    ...
    $roomCategory = $this->adminRoomCategoryOrFail($request->roomCategoryId);
    $roomRent = $roomCategory->rent;
    ...
    return view('admin.rooms.booking.available-room', [
